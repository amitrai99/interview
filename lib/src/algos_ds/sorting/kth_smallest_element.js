 ...
 */
function quickSelect(items, si, ei, k, kthItem) {
  ...
  if( k <= ei - si + 1) {
    ...
    if(k === pIdx) {
      ...
      return quickSelect(items, si, pIdx-1, k, kthItem);
    } else if ( k > pIdx ) {
      //kth smallest element is in the upper array
      return quickSelect(items, pIdx+1, ei, k, kthItem);
    }
...
function test() {
  var items = [33,11,10,14],
      k = 2,
      kthItem;

  console.log('find the %d th smallest item in array : ', k, items.toString() );
  kthItem = quickSelect(items, 0, items.length - 1, k-1, kthItem);
  console.log(kthItem);
}
